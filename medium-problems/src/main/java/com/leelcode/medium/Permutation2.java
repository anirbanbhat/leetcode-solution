...
public class Permutation2 {
	...
	public static void main(String[] args) {
		...
		ArrayList<ArrayList<Integer>> result = permutation(new int[]{1, 2, 3, 4}, lst, 0);
		
		System.out.println("************RESULT*************");
		printList(result);
	}
	
	public static ArrayList<ArrayList<Integer>> permutation(int[] nums, ArrayList<ArrayList<Integer>> lst, int start){
		
		ArrayList<Integer> dummy = new ArrayList<>();
		
		for (int i = 0; i < nums.length; i++) {
			ArrayList<ArrayList<Integer>> current = new ArrayList<>();
			for(ArrayList<Integer> arr: lst) {
